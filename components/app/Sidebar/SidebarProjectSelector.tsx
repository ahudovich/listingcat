...
const currentProject = {
  websiteUrl: 'https://blueskymeter.com',
  name: 'Bluesky Meter',
}
...
const projects = [
  ...
  {
    name: 'Listing Cat',
    websiteUrl: 'https://listingcat.com',
    slug: 'listingcat',
  },
  {
    name: 'Affonso',
    websiteUrl: 'https://affonso.io',
    slug: 'affonso',
  },
]
        ...
        >
          <Menu.Popup className="overflow-hidden p-1 w-full bg-white border border-zinc-200 rounded-lg shadow-lg transition-[transform,scale,opacity] data-[starting-style]:scale-90 data-[starting-style]:opacity-0">
            {projects.map((project) => (
              <SidebarProjectSelectorItem key={project.websiteUrl} project={project} />
            ))}
          </Menu.Popup>
        </Menu.Positioner>
      </Menu.Portal>
...
export function SidebarProjectSelectorItem({ project }: { project: (typeof projects)[number] }) {
  return (
    <Menu.Item
